...
def SubmitData():
    ...
    elif ((AGE.get()).isnumeric())==False:
        result = tkMessageBox.showwarning('', 'Age is in numeric value', icon="warning")
    elif ((CONTACT.get()).isnumeric())==False:
        result = tkMessageBox.showwarning('', 'contact is in numeric value', icon="warning")
    else:
...
def UpdateData():
    if GENDER.get() == "":
       result = tkMessageBox.showwarning('', 'Please choose the gender option', icon="warning")
    elif ((AGE.get()).isnumeric())==False:
        result = tkMessageBox.showwarning('', 'Age is in numeric value', icon="warning")
    elif ((CONTACT.get()).isnumeric())==False:
        result = tkMessageBox.showwarning('', 'contact is in numeric value', icon="warning")
        ...
        conn = sqlite3.connect("pythontut.db")
        cursor = conn.cursor()
        cursor.execute("UPDATE `member` SET `firstname` = ?, `lastname` = ?, `gender` =?, `age` = ?,  `address` = ?, `contact` = ? WHERE `mem_id` = ?", (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), str(AGE.get()), str(ADDRESS.get()), int(CONTACT.get()), int(mem_id)))
        conn.commit()
        cursor.execute("SELECT * FROM `member` ORDER BY `lastname` ASC")
        fetch = cursor.fetchall()
        for data in fetch:
...
def OnSelected(event):
    ...
    lastname = Entry(ContactForm, textvariable=LASTNAME, font=('arial', 14),bd=3)
    lastname.grid(row=1, column=1)
    RadioGroup.grid(row=2, column=1)
    age = Entry(ContactForm, textvariable=AGE,  font=('arial', 14),bd=3)
    age.grid(row=3, column=1)
    address = Entry(ContactForm, textvariable=ADDRESS,  font=('arial', 14),bd=3)
    address.grid(row=4, column=1)
    contact = Entry(ContactForm, textvariable=CONTACT,  font=('arial', 14),bd=3)
...
def AddNewWindow():
    ...
    def temp_text(e):
        age.delete(0,"end")
    age.insert(0,"In_numeric_value...")
    age.bind("<FocusIn>",temp_text)
    age.grid(row=3, column=1)
    address = Entry(ContactForm, textvariable=ADDRESS,  font=('arial', 14),bd=3)
    address.grid(row=4, column=1)
    contact = Entry(ContactForm, textvariable=CONTACT,  font=('arial', 14),bd=3)
    ...
    contact.insert(0,"In_numeric_value...")
    contact.bind("<FocusIn>",temporary)
    contact.grid(row=5, column=1)
